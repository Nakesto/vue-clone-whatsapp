<template>
    ...
    <q-drawer class="rspsv" v-model="leftDrawerOpen" :width="style.width > 1100 ? 480 : style.width > 800 ? 300 : 280"
        show-if-above bordered :breakpoint="700">
                    ...
                    <q-item-section side>
                        <q-item-label caption>
                            {{ new Date(conversation.last_message_time).getHours() }}:{{
                                new
                                                        Date(conversation.last_message_time).getMinutes()
                            }}
                        </q-item-label>
        ...
        <AddComponent v-if="addDrawerOpen" :handleClose="handleClose" />
    </q-drawer>

    <q-page-container class="bg-grey-2" :style="style.width > 690 ? {
    ...
    <q-footer v-if="currentConversationIndex !== null">
        <q-form style="width: 100%;" @submit.prevent="sendMessage">
            <q-toolbar class="bg-grey-3 text-black row">
                ...
                <q-input color="teal" rounded outlined dense class="WAL__field col-grow q-mr-sm" bg-color="white"
                    v-model="message" placeholder="Type a message" />
                <q-btn v-if="message !== ''" type="submit" round color="secondary" icon="send"></q-btn>
...
const fetchData = async () => {
    try {
        ...
        conversations.value = result.data.chatroom
    } catch (err) {
        console.log(err)
    }
}
    ...
    if (socket.value !== null) {
        ...
        socket.value.onmessage = function (evt) {
            ...
            event.forEach((result) => {
                if (result.type === "room") {
                    conversations.value = conversations.value.map((chat) => result.data.ReceiveName == chat.ReceiveName || result.data.ReceiveName == chat.SenderName ? {
                        ...chat, lastmessage: result.data.lastmessage, last_message_time: result.data.last_message_time
                    } : chat)
                }
